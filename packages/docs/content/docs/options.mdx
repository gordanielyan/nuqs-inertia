...
When using an Inertia adapter, nuqs can also carry partial reload options
through hook-level and call-level updates:
...
Like the other options, call-level values override hook-level defaults.
